...
void main() {
  ...
  final expectedMaleGroupLabel = "юноши 14-15 лет";
  final expectedFemaleGroupLabel = "девушки 14-15 лет";

  final expectedAgeLabel = "14-15 лет";

  group("male sports category", () {
    late SportsCategorySet set;

    setUp(() {
      set = KyokushinYouthsSportCategorySetStrategy().createSet(isMale: true);
    });
    ...
    test("set should have one 40kg, one 45kg, one 50kg, one 55kg, one 60kg, one 65kg BelowWeightCategory", () {
    ...
    test("set should have one 65+kg AboveWeightCategory", () {
  ...
  group("female sports category", () {
    ...
    setUp(() {
      set = KyokushinYouthsSportCategorySetStrategy().createSet(isMale: false);
    });
    ...
    test("set should have one 45kg, 50kg, 55kg, 60kg BelowWeightCategory", () {
      expect(getAllWeightsInBelowWeightCategories(set.weightCategories, 45), 1);
      expect(getAllWeightsInBelowWeightCategories(set.weightCategories, 50), 1);
      expect(getAllWeightsInBelowWeightCategories(set.weightCategories, 55), 1);
      expect(getAllWeightsInBelowWeightCategories(set.weightCategories, 60), 1);
