...
void main() {
  ...
  final expectedMaleGroupLabel = "юноши 12-13 лет";
  final expectedFemaleGroupLabel = "девушки 12-13 лет";

  final expectedAgeLabel = "12-13 лет";

  group("male sports category", () {
    late SportsCategorySet set;

    setUp(() {
      set = KyokushinTeenagersSportCategorySetStrategy().createSet(
        isMale: true,
      );
    });
    ...
    test("set should have 5 BelowWeightCategory", () {
    ...
    test(
      "set should have one 35kg, one 40kg, one 45kg, one 50kg, one 55kg BelowWeightCategory",
      () {
        expect(
          getAllWeightsInBelowWeightCategories(set.weightCategories, 35),
          1,
        );
        expect(
          getAllWeightsInBelowWeightCategories(set.weightCategories, 40),
          1,
        );
        expect(
          getAllWeightsInBelowWeightCategories(set.weightCategories, 45),
          1,
        );
        expect(
          getAllWeightsInBelowWeightCategories(set.weightCategories, 50),
          1,
        );
        expect(
          getAllWeightsInBelowWeightCategories(set.weightCategories, 55),
          1,
        );
      },
    );
  ...
  group("female sports category", () {
    ...
    test("set should have 5 weight categories", () {
      expect(set.weightCategories.length, 5);
    });

    test("set should have 4 BelowWeightCategory", () {
      expect(getBelowWeightCategoriesCount(set.weightCategories), 4);
    });
    ...
    test(
      "set should have one 40kg, 45kg, 50kg, 55kg BelowWeightCategory",
      () {
