...
void main() {
  ...
  PersonName parseName(String nameWithInitials) {
    return NameWithInitialsParser().parse(nameWithInitials);
  }
  ...
  group("TrainersList_Success", () {
    ...
    group("add", () {
      test("add trainer to list should add trainer and increment size", () {
        final id = Id();
        final given = Trainer(id: id, name: parseName("Пупкин И.А."));
    ...
    group("toString", () {
      test("toString should return string of all trainers name, concatenated with ','", () {
        final trainersList = TrainersListBasicImpl();

        final given1 = Trainer(id: Id(), name: parseName("Пупкин И.А."));
        final given2 = Trainer(id: Id(), name: parseName("Васильев В.В."));
